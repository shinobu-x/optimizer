...
class SharedLRSchedulerAdam(Adam):
    def __init__(self, params, max_step, lr = 1e-3, betas = (0.9, 0.999),
            eps = 1e-3, weight_decay = 0.0, lrs = [1e-4, 1e-4 * 0.9, 1e-4 * 0.8,
                1e-4 * 0.7, 1e-4 * 0.6, 1e-4 * 0.5, 1e-4 * 0.4, 1e-4 * 0.3,
                1e-4 * 0.2, 1e-4 * 0.1], num_steps = 10):
        super(SharedLRSchedulerAdam, self).__init__(params, lr, betas, eps,
                weight_decay)
    ...
    def step(self, closure = None):
        ...
        if closure is not None:
            loss = closure()
        lr = self.lrs[int(state['step'][0] //
            (self.max_step // self.num_steps))]
        group['lr'] = lr
        for group in self.param_groups:
            for p in group['params']:
                if p.grad is None:
